...
async function home(req, res) {
  try {
    ...
    return res.render('shop/home', { featured, demoMode: false });
  } catch (error) {
    if (!isDbUnavailable(error)) throw error;
    return res.render('shop/home', { featured: mockData.products, demoMode: true });
  }
}
...
async function products(req, res) {
  ...
  try {
    ...
    return res.render('shop/products', { categories, items, filters: { category, q, sort }, demoMode: false });
  } catch (error) {
    if (!isDbUnavailable(error)) throw error;

    let items = [...mockData.products];
    if (category) items = items.filter((p) => mockData.categories.find((c) => c.id === p.category_id)?.slug === category);
    if (q) items = items.filter((p) => `${p.name} ${p.short_description}`.toLowerCase().includes(String(q).toLowerCase()));
...
async function productDetail(req, res) {
  ...
  try {
    ...
    const product = mockData.findProductBySlug(slug);
    if (!product) return res.status(404).render('shop/error', { message: 'Product not found' });

    return res.render('shop/product-detail', {
      product,
      images: [{ image_url: product.image_url, alt_text: product.name }],
